...
\begin{figure}[H]
    \centering  
    \includegraphics[scale=0.6]{sparkarc}  
...
\textit{Cluster Manager} berfungsi mengatur sumber daya pada sebuah \textit{cluster}. Spark dapat berjalan pada berbagai macam \texit{cluster manager} seperti Apache Mesos, Hadoop YARN, dan \textit{Spark's stand alone}. \textit{Cluster manager} akan berusaha mendapatkan sumber daya pada \texit{cluster} dan mealokasikannya kepada \textit{Spark job} yang sedang berjalan.
...
\item \textbf{Tasks}\\
\textit{Task} adalah sebuah satuan kerja pada Spark. \textit{Task} berisi perintah. Perintah tersebut merupakan fungsi yang diserialisasi. \textit{Task} akan dikirimkan oleh \textit{driver program} kepada \texit{executor}. Kemudian, \textit{executor} akan medeserialisai perintah tersebut dan mengerjakannya. Biasanya \textit{task} akan dibuat untuk setiap partisi. Partisi merupakan potongan data yang terdistribusi pada \textit{cluster}.
